...
<li class="nav-item me-3">
  <div style="position: relative; width: 200px;"> 
    <input type="text" id="userSearch" class="form-control form-control-sm" placeholder="Search..." style="border-radius: 10px; padding-right: 30px; height: 30px; font-size: 0.85rem;">
    <i class="bi bi-search" style="position: absolute; right: 8px; top: 50%; transform: translateY(-50%); color: #6c757d; font-size: 0.9rem;"></i>
  </div>
</li>
...
<td>
<span class="badge rounded-pill <?= $row['role']==='admin'?'badge-admin':'badge-user' ?>">
<?= htmlspecialchars($row['role']) ?>
</span>
</td>
<td><?= date("Y-m-d | h:i A", strtotime($row['created_at'])) ?></td>
<td>
<span class="badge rounded-pill <?= $row['status']==='active'?'badge-active':'badge-inactive' ?>">
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    document.querySelectorAll('.password-text .eye-icon').forEach(icon=>{
        icon.addEventListener('click',()=>{
            ...
            if(span.textContent==='******'){
                span.textContent=password; icon.classList.replace('bi-eye','bi-eye-slash');
            } else{
                span.textContent='******'; icon.classList.replace('bi-eye-slash','bi-eye');
            }
        });
    });
