...
$search_sql = '';
if($search){
    $search_sql = "WHERE name_en LIKE ? OR employee_id_no LIKE ? OR position LIKE ?";
}

if($search_sql){
    $stmt = $conn->prepare("SELECT * FROM profile $search_sql ORDER BY id ASC");
    $like = "%$search%";
    $stmt->bind_param("sss",$like,$like,$like);
    $stmt->execute();
    $result = $stmt->get_result();
} else {
    $result = $conn->query("SELECT * FROM profile ORDER BY id ASC");
}
?>
<!DOCTYPE html>
<html>
        ...
        <ul class="navbar-nav">
            ...
            <li class="nav-item dropdown">
                ...
                <ul class="dropdown-menu dropdown-menu-end">
                    ...
                    <!-- employee -->
                     <li>
                        <a class="dropdown-item" href="../employees/employee_list.php"><i class="bi bi-people-fill me-2 text-warning"></i>Employees</a>
                     </li>
                    ...
                    <li><a class="dropdown-item" href="../users/user_list.php"><i class="bi bi-people-fill me-2 text-danger"></i>Users</a></li>
                        ...
                        <li>
                                <a class="dropdown-item" href="../home/index.php"><i class="bi bi-house-door-fill me-2 text-success"></i>Home</a>
                    <?php endif; ?>
    ...
    <table class="table table-hover table-bordered align-middle">
        ...
        <?php if($result->num_rows > 0):
            ...
            <tr>
                ...
                <td data-tooltip="<?= htmlspecialchars($row['name_en']) ?>" class="text-truncate"><?= htmlspecialchars($row['name_en']) ?></td>
                <td data-tooltip="<?= htmlspecialchars($row['employee_id_no']) ?>" class="text-truncate"><?= htmlspecialchars($row['employee_id_no']) ?></td>
                <td data-tooltip="<?= htmlspecialchars($row['position']) ?>" class="text-truncate"><?= htmlspecialchars($row['position']) ?></td>
                <td><?= htmlspecialchars($row['gender']) ?></td>
                <td><?= $row['birth_date'] ? date('d-M-Y', strtotime($row['birth_date'])) : '-' ?></td>
                <td><?= htmlspecialchars($row['phone']) ?></td>
                <td data-tooltip="<?= htmlspecialchars($row['email']) ?>" class="text-truncate"><?= htmlspecialchars($row['email']) ?></td>
                ...
                <td data-tooltip="<?= htmlspecialchars($row['current_address']) ?>" class="text-truncate"><?= htmlspecialchars($row['current_address']) ?></td>
                <td><a href="edit_profile.php?id=<?= $row['id'] ?>" class="btn btn-edit btn-sm"><i class="fas fa-edit"></i> Edit</a></td>
            </tr>
        <?php endwhile; else: ?>
            <tr><td colspan="15" class="text-center">No profiles found.</td></tr>
        <?php endif; ?>
        </tbody>
    </table>
</div>
